...
def create_maturity_gauge(score):
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            'bordercolor': "black",
            'steps': [
                {'range': [5, 12], 'color': '#ff6b6b'},  # Darker red
                {'range': [12, 19], 'color': '#ffda77'}, # Darker yellow
                {'range': [19, 25], 'color': '#4caf50'}  # Darker green
            ],
        }
    ))
...
# Function to generate PDF reportfrom io import BytesIO
from datetime import datetime
from reportlab.lib.pagesizes import letter
...
def generate_pdf_report(df, company_name):
    ...
    # Filter competitors to only those in the same industry
    industry_competitors = competitors[competitors['Primary_Industry'] == company_industry].copy()

    # Combine the main company and its industry-specific competitors
    ...
    # Clean company names for consistent ranking
    ...
    # Revenue efficiency check (only among same-industry competitors)
...
#     # Add tabs content using create_time_series_plot, etc.
